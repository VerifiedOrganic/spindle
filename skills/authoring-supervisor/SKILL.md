...
- The tool also works when the block is RUN-level (reconcile findings such as summary/phase residue) rather than a per-scene `blocked_reason`: forward advances are allowed on scenes in a blocked run, and pre-existing-prose scenes without a run artifact may still be advanced. A truly unblocked scene in an unblocked run is refused.
- A chapter that already has persisted prose and a persisted summary from outside the run is ADOPTED on the first execute (scenes marked `beats_annotated`, `summary_saved` set) instead of blocking; adopted scenes skip in-run commit/mine/beats, so mine adopted scenes manually if you want canon deltas from them.
- Re-saving a scene via `authoring_save_scene_draft` in a chapter whose summary is already persisted (the revise-during-checkpoint flow) automatically deletes that chapter's summary row and artifact and resets `summary_saved`, so the pipeline regenerates the summary after the revised scene recommits.
